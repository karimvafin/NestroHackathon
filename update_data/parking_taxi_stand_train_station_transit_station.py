...
def get_parking_taxi_stand_train_station_transit_stations(adress: str, LAT: list):
    url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json"

    params = {
        "location": adress,
        "radius": 50000,
        "type": ["parking", "taxi_stand", "train_station", "transit_station"],
        "key": "AIzaSyCjgcE4I9N0VYO9nTA2XHBgaoeM6Glnzww"
    }

    response = requests.get(url, params=params)
    parking_taxi_stand_train_station_transit_stations = []
    ...
    for result in data['results']:
        ...
        if is_in_bosnia(lat=lat, lng=lng) and rating is not None and (lat, lng) not in LAT:
            LAT.append((lat, lng))
            parking_taxi_stand_train_station_transit_stations.append(
    ...
    return [parking_taxi_stand_train_station_transit_stations, LAT]


# Вызовите функцию для получения списка заправок
def update_parking_taxi_stand_train_station_transit_stations():
    ...
    for adress in list_adress:
        parking_taxi_stand_train_station_transit_station_result = get_parking_taxi_stand_train_station_transit_stations(adress, lat)
        lat = parking_taxi_stand_train_station_transit_station_result[1]
        parking_taxi_stand_train_station_transit_stations.extend(parking_taxi_stand_train_station_transit_station_result[0])
        print("len", len(parking_taxi_stand_train_station_transit_stations))
    ...
    for diction in parking_taxi_stand_train_station_transit_stations:
    ...
    new_file_path = 'data/origin/parking|taxi_stand|train_station|transit_station.xlsx'
